...
#include "cute_voice/qtts.h"
#include "cute_voice/msp_cmn.h"
#include "cute_voice/msp_errors.h"
...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    {
        char nameString[40] = "您好，主人，我是小牧。";
        text = nameString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.compare("你准备好了吗？") == 0)
    {
        char readyString[40] = "时刻准备着。";
        text = readyString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.compare("向前。") == 0 || dataString.compare("向后。") == 0\
		 || dataString.compare("向左。") == 0 || dataString.compare("向右。") == 0\
		 || dataString.compare("立正。") == 0 || dataString.compare("停止。") == 0)
    {
        char startString[40] = "收到。";
        text = startString;
        std::cout<<text<<std::endl;
    }
...
{
	int         ret                  = MSP_SUCCESS;
	const char* login_params         = "appid = 5de4c591, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
	/*
	* rdn:           合成音频数字发音方式
	* volume:        合成音频的音量
	* pitch:         合成音频的音调
